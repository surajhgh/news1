...
const PostDetails = () => {
    ...
    const fetchPost = async () => {
      try {
        ...
        if (res.ok) {
          ...
          setLoading(false);
          setError(false);
          // Fetch recommended articles after post is loaded
          fetchRecommendedArticles(data.posts[0].category);
        }
      } catch (error) {
        setError(true);
        setLoading(false);
  ...
  const fetchRecommendedArticles = async (category) => {
    try {
      ...
      if (res.ok) {
        ...
        const filteredArticles = data.posts.filter(
  ...
  return (
    <main className="p-3 flex flex-col max-w-6xl mx-auto min-h-screen">
      ...
      <div className="flex flex-col justify-center items-center mb-5">
        <h1 className="text-xl font-semibold mt-5 text-slate-700">
          Recommended {post && post.category} articles
        </h1>
